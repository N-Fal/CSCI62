...
{
    // network setup (add error checks)
    char users[] = "C:/cygwin64/home/sidek/CSCI62/Homework4/Network/userlist.txt", posts[] = "C:/cygwin64/home/sidek/CSCI62/Homework4/Network/posts.txt";
    net.readUsers(users);
    net.readPosts(posts);
...
void SocialNetworkWindow::populateProfile(User* u)
{
    ...
    // populate the friend list (if the user is on the home page)
    {
        if (otherUser->getId() == loggedUser->getId())
        {
            ...
            ui->suggestedLabel->setText(QString::fromStdString("These users have " + std::to_string(score) + " friends in common with you! Click one to add them as a friend."));
...
{
    char users[] = "C:/cygwin64/home/sidek/CSCI62/Homework4/Network/userlist.txt";
