...
void printMenu()
{
    ...
    cout << "(11 'name1' 'num') - prints the 'num' most recent posts on a user's page" << endl;
...
int main(int argc, char *argv[])
{
    ...
    cout << "loading " << usersFile << " ..." << endl;

    Network network;

    if (network.readUsers(usersFile) == -1)
    {
        cout << "The file " << usersFile << " could not be found in the directory. Please make sure you entered the name correctly and try again";
        return 2;
    }
    
    cout << usersFile << " is loaded into the network." << endl << endl;

    char* postsFile = argv[2];
    cout << "loading " << postsFile << " ..." << endl;

    if (network.readPosts(postsFile) == -1)
    {
        cout << "The file" << postsFile << " could not be found in the directory. Please make sure you entered the name correctly and try again";
        return 2;
    }

    cout << postsFile << " is loaded into the network." << endl << endl;
    ...
    while (true)
    {
        ...
        switch (input)
        {
            ...
            case 11:
            {
                cin >> first1 >> last1 >> recent;
                // "For now you should show all Posts and all public AND private IncomingPosts"
                cout << network.getPostsString(network.getId(first1 + " " + last1), recent, false);
                break;
            }
            default:
